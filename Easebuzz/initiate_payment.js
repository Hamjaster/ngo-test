...
let initiate_payment = function (data, config, res) {
  ...
  var hash_key = generateHash(data.key, data.tnxid, data.amount, data.productinfo, data.name, data.email, config.salt);
  ...
  util.call(call_url, form()).then(function (response) {
    console.log(response.data, 'access KEY')
  ...
  //   const encryptedString = sha512.digest('hex');
